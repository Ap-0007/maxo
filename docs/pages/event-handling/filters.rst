...
Фильтр всегда получает два позиционных аргумента: ``update`` и ``ctx``. Чтобы
развернуть значения из ``ctx`` в именованные параметры, примените к ``__call__``
декоратор :func:`maxo.routing.utils.inline_ctx`.
...
Декоратор передаст ``bot`` и ``fsm_context`` из ``ctx`` по именам. Его также
можно применять к обработчикам, когда зависимости из ``ctx`` нужны в их
собственном callback.
