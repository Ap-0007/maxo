...
Только ``update`` передается фильтру позиционно. ``ctx`` и любое другое значение
из контекста объявляются именованными параметрами ``__call__`` - **maxo**
подставит их сам, как это делается для обработчиков. Так фильтр получает ``bot``,
``fsm_context``, ``event_from_user`` или зависимость, положенную мидлварью, не
доставая ее из ``ctx`` руками.
...
``FilterObject`` передает ``update`` позиционно, а ``ctx`` и остальные
зависимости - по именам. Поэтому объявлять зависимости можно обычными
параметрами, ``mypy`` на это не ругается.
