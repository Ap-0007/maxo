...
.. note::
    ...
    Декоратор флага навешивает атрибут на функцию, поэтому он должен стоять
    **ниже** декоратора регистрации хендлера.
...
Если один и тот же флаг задан несколькими способами, побеждает более конкретный:

1. ``flags={...}`` при регистрации хендлера,
2. флаги от фильтров,
3. флаги от декораторов на функции.

Как прочитать флаг
------------------

Флаги доступны в фильтрах и **inner**-мидлварях: к моменту их выполнения хендлер
уже выбран и лежит в ``ctx`` под ключом ``handler``. В **outer**-мидлварях
