...
Комбинаторы ``&`` и ``|`` пробрасывают флаги вложенных фильтров, поэтому
``Command("start") | Command("help")`` заполнит флаг ``commands`` обеими
командами. Инверсия ``~`` флаги не пробрасывает: она меняет смысл фильтра на
противоположный, и хендлер с ``~Command("start")`` обрабатывает всё, кроме
``/start``, так что во флаге ``commands`` этой команде не место.
