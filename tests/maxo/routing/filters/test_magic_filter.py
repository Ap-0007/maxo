...
class TestMagicFilter:
    ...
    async def test_rejects_falsy_result(self) -> None:
        assert await MagicFilter(F["flag"])({"flag": False}, {}) is False  # type: ignore[arg-type]

    async def test_accepts_truthy_result(self) -> None:
        assert await MagicFilter(F["flag"])({"flag": True}, {}) is True  # type: ignore[arg-type]

    async def test_stores_result_in_ctx(self) -> None:
        ctx: dict[str, Any] = {}

        assert await MagicFilter(F["value"], result_key="found")({"value": 7}, ctx)  # type: ignore[arg-type]
        assert ctx["found"] == 7
